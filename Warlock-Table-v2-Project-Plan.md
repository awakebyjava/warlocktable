...
| **HDMI handshake** | Boot with the TV off → bad resolution or no output when it wakes. **This happened for real on 2026-08-20:** a power cut while the TCL was off left X at a 1024×768 fallback with the *disconnected* HDMI port set as primary. The table screen showed nothing and input could not fix it — the Pi was not blanking, it was driving the wrong output. | **Fixed.** The TCL is on the Pi's second HDMI port, so `/boot/config.txt` needs the port suffix: `hdmi_force_hotplug:1=1`, `hdmi_group:1=1`, `hdmi_mode:1=16` (CEA 1080p60). Backup at `/boot/config.txt.bak-*`. To repair a running session: `xrandr --output HDMI-2 --mode 1920x1080 --rate 60 --primary`. |
